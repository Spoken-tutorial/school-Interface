...
class OrganisationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Organisation
        fields = ['id','name_of_association','date_of_association','type','updated','location','added_by','is_active']
        
    def create(self, validated_data):
        user = saveUser(validated_data.pop('added_by')) # save the nested user instance
        location = saveLocation(validated_data.pop('location')) # save the nested location instance
        organisation = Organisation.objects.create(**validated_data,added_by=user,location=location)
        return organisation
...
class SchoolSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = School
        fields = ['id','name_of_association','date_of_association','type','organisation','organisation_id','updated','is_active','added_by','location']
